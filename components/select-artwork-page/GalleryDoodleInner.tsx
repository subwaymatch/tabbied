...
export default function GalleryDoodleInner({ item }: { item: GalleryItem }) {
  ...
  // rotates it from there (pausing for reduced motion / hidden tabs). The
  // cover fit reproduces the fixed-resolution + transform-scale technique, so
  // fixed-px strokes and shadows keep the proportions of the original 800px
  // artwork at any card size.
  return (
    <TabbiedArtwork
      artwork={item.slug}
