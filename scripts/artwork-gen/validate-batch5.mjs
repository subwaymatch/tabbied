// Renders every batch-5 artwork in headless Chromium at its gallery-thumbnail
...
// Also writes contact-sheet screenshots to /tmp/sheet-b5-*.png for review.
...
const CHUNK = 20;
const chunks = [];
for (let i = 0; i < batch5.length; i += CHUNK) chunks.push(batch5.slice(i, i + CHUNK));
...
for (const chunk of chunks) {
  ...
  await page.screenshot({ path: `/tmp/sheet-b5-${shot}-seed1.png`, fullPage: true });
  ...
  await page.screenshot({ path: `/tmp/sheet-b5-${shot}-seed2.png`, fullPage: true });
...
console.log(`all ${batch5.length} batch-5 artworks render, reseed and keep their cells ✓`);
